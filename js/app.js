...
import { estimateGaze,getDirection,smoothGaze,estimateEyeOpenness } from './gazeEstimator.js?v=20260912-1';
import { Calibration,mapGaze } from './calibration.js';
import { DwellSelection } from './dwellSelection.js';
import { BlinkDetector } from './blinkDetector.js?v=20260912-1';
import { ScanningMode } from './scanningMode.js?v=20260912-1';
...
import { setupSettings } from './settings.js?v=20260912-1';
...
let quickIndex=0;
const settings=setupSettings(()=>calibration.start(),setScanning); const tts=new TextToSpeech(); document.querySelector('#language').onchange=()=>tts.populate();
...
const scanner=new ScanningMode(()=>Array.from(document.querySelectorAll('.direction')));
const blink=new BlinkDetector(()=>{const target=scanner.current();if(!target)return;dwell.clear();flashSelected(target);action(target);});
function setScanning(on){if(on){dwell.clear();updateDirections('CENTER');scanner.start(settings.dwell);}else{scanner.stop();blink.reset();}}
if(settings.scanning)setScanning(true);
